...
const LaunchpadPreparationSubDetailView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        "Target Funding Amount ($)": fundTarget,
        "Desired Launchpad Platforms": targetPlatforms,
        "Type": "Launchpad Preparation Sub-Detail",
      ...
      const result = await evaluateProject('Launchpad Prep', `Target Fund: ${fundTarget}. Platforms: ${targetPlatforms}`);
      setAiResult(result);
    } catch (err: any) {
      console.error(err);
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style">{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Reset</button></div> : (
                <form onSubmit={handleSubmit}>
                  <p className="text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">Fund Target ($)*</p>
                  <input 
                    type="text" 
                    ...
                    required 
                  />
                  <p className="text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">Target Platforms*</p>
                  <textarea 
                    className="w-full border p-3 rounded-lg mb-4" 
                    rows={3} 
                    placeholder="Which platforms are you targeting? (e.g. Seedify, DAO Maker)" 
                    value={targetPlatforms}
                    onChange={(e) => setTargetPlatforms(e.target.value)}
                    required 
                  />
                  <button type="submit" disabled={loading} className="form-button">
                    {loading ? 'OPENING TELEGRAM...' : 'GET PREP PLAN'}
                  </button>
                </form>
              )}
            </div>
