...
const ServerInfrastructureSubDetailView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        "Infras Need": status,
        "Load/Network Detail": goal,
        "Type": "Server Infrastructure Sub-Detail",
  ...
  return (
    <div className="crypto-detail-page">
      ...
      <section id="h-hero">
        <img src="https://images.unsplash.com/photo-1558494949-ef010cbdcc51?q=80&w=2832" className="bg-img" alt="Server Infrastructure" />
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style" style={{color: '#000'}}>{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Try Again</button></div> : (
                <form onSubmit={handleSubmit}>
                  ...
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Load/Network Detail*</p>
                  ...
                  <button type="submit" disabled={loading} className="form-button">{loading ? 'OPENING TELEGRAM...' : 'GET INFRASTRUCTURE PLAN'}</button>
                </form>
              )}
            </div>
          </div>
...
export default ServerInfrastructureSubDetailView;