...
const SocialMediaVisualPackagesSubDetailView: React.FC = () => {
  ...
  return (
    <div className="crypto-detail-page">
      ...
      <section id="h-hero">
        <img src="https://images.unsplash.com/photo-1558655146-d09347e92766?q=80&w=2832" className="bg-img" alt="Visual Asset Hero" />
        ...
        <div className="container-xl">
          <div className="hero-grid">
            <div>
              ...
              <p className="p-style">Transform your social channels into a high-end digital showroom. We provide a complete arsenal of visual assets tailored for the Web3 aesthetic.</p>
            </div>
            <div className="form-card">
              <h3 style={{textAlign: 'center', marginBottom: '20px', fontWeight: 800}}>Package Analysis</h3>
              {aiResult ? <div className="p-style">{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Reset</button></div> : (
                <form onSubmit={handleSubmit}>
                  <input type="text" className="w-full border p-3 rounded-lg mb-4" placeholder="Primary Channel" required />
                  <textarea className="w-full border p-3 rounded-lg mb-4" rows={3} placeholder="What is your current design bottleneck?" required />
                  <button type="submit" disabled={loading} className="form-button">{loading ? 'DESIGNING...' : 'GET ASSET PLAN'}</button>
                </form>
              )}
            </div>
          </div>
