...
const TelegramBotMiniAppSubDetailView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        "App Type": status,
        "Viral Mechanism": goal,
        "Type": "Telegram Mini App Sub-Detail",
      ...
      const result = await evaluateProject(status, `Telegram Mini App: ${goal}`);
      setAiResult(result);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="crypto-detail-page">
      ...
      <section id="h-hero">
        <img src="https://images.unsplash.com/photo-1614680376593-902f74cf0d41?q=80&w=2832" className="bg-img" alt="Telegram App" />
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style" style={{color: '#000'}}>{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Try Again</button></div> : (
                <form onSubmit={handleSubmit}>
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Type*</p>
                  <select className="form-control" value={status} onChange={e=>setStatus(e.target.value)} required>
                    <option value="">Need</option><option value="tma">Mini App (TMA)</option><option value="bot">Trading Bot</option>
                  </select>
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Viral Mechanism*</p>
                  ...
                  <button type="submit" disabled={loading} className="form-button">{loading ? 'OPENING TELEGRAM...' : 'GET TELEGRAM PLAN'}</button>
                </form>
              )}
            </div>
          </div>
...
export default TelegramBotMiniAppSubDetailView;