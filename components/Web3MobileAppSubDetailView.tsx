  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        ...
        "Mobile Goals": goal,
        "Type": "Web3 Mobile App Sub-Detail",
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style" style={{color: '#000'}}>{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Try Again</button></div> : (
                <form onSubmit={handleSubmit}>
                  ...
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Mobile Goals*</p>
                  ...
                  <button type="submit" disabled={loading} className="form-button">{loading ? 'OPENING TELEGRAM...' : 'GET MOBILE PLAN'}</button>
                </form>
              )}
            </div>
          </div>
