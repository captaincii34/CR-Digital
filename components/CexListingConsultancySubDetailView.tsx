...
const CexListingConsultancySubDetailView: React.FC = () => {
  ...
  const [exchangeGoal, setExchangeGoal] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        "Target Exchanges": exchangeGoal,
        "Current Stats (Holders/Volume)": stats,
        "Type": "CEX Listing Consulting Sub-Detail",
      ...
      const result = await evaluateProject('CEX Listing', `Exchange: ${exchangeGoal}. Stats: ${stats}`);
      setAiResult(result);
    } catch (err: any) {
      console.error(err);
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style">{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Reset</button></div> : (
                <form onSubmit={handleSubmit}>
                  <p className="text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">Target Exchange*</p>
                  <input 
                    type="text" 
                    className="w-full border p-3 rounded-lg mb-4" 
                    placeholder="e.g. Gate.io, Bybit, Binance" 
                    value={exchangeGoal}
                    onChange={(e) => setExchangeGoal(e.target.value)}
                    required 
                  />
                  <p className="text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">Metrics & Stats*</p>
                  <textarea 
                    className="w-full border p-3 rounded-lg mb-4" 
                    rows={3} 
                    placeholder="Share your current holder and volume data..." 
                    value={stats}
                    onChange={(e) => setStats(e.target.value)}
                    required 
                  />
                  <button type="submit" disabled={loading} className="form-button">
                    {loading ? 'OPENING TELEGRAM...' : 'GET LISTING PLAN'}
                  </button>
                </form>
              )}
            </div>
