...
const GameFiP2ESystemsSubDetailView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        "Game Model": status,
        "Reward Mechanism": goal,
        "Type": "GameFi / P2E Systems Sub-Detail",
      ...
      const result = await evaluateProject(status, `GameFi System: ${goal}`);
      setAiResult(result);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="crypto-detail-page">
      ...
      <section id="h-hero">
        <img src="https://images.unsplash.com/photo-1621761191319-c6fb62004040?q=80&w=2832" className="bg-img" alt="GameFi" />
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style" style={{color: '#000'}}>{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Reset</button></div> : (
                <form onSubmit={handleSubmit}>
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Model*</p>
                  <select className="form-control" value={status} onChange={e=>setStatus(e.target.value)} required>
                    <option value="">Model</option><option value="p2e">Play-to-Earn</option><option value="m2e">Move-to-Earn</option>
                  </select>
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Reward Mechanism*</p>
                  ...
                  <button type="submit" disabled={loading} className="form-button">{loading ? 'OPENING TELEGRAM...' : 'GET ECONOMY PLAN'}</button>
                </form>
              )}
            </div>
          </div>
...
export default GameFiP2ESystemsSubDetailView;