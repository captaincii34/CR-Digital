  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const statusLabels: { [key: string]: string } = {
        'new': 'New Project',
        'current': 'Current Project Optimization'
      };

      const formObj = {
        "Need Status": statusLabels[status] || status,
        "Project Goal Description": goal,
        "Type": "Token Project Consulting Sub-Detail",
  ...
  return (
    <div className="crypto-detail-page">
        ...
        <div className="container-xl">
          <div className="hero-grid">
            <div style={{flex: 1.2}}>
              ...
              <p className="p-style">We transform your idea into a technically perfect blockchain project that is market standard and investor friendly. Minimize risks and maximize gains with our deep experience in the industry.</p>
            </div>
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style" style={{color: '#000'}}>{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Try Again</button></div> : (
                <form onSubmit={handleSubmit}>
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Need Status*</p>
                  <select className="form-control" value={status} onChange={e=>setStatus(e.target.value)} required>
                    <option value="">Select Status</option>
                    <option value="new">New Project</option>
                    <option value="current">Current Project Optimization</option>
                  </select>
                  <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-widest">Goal Summary*</p>
                  ...
                  <button type="submit" disabled={loading} className="form-button">{loading ? 'OPENING TELEGRAM...' : 'GET ANALYSIS REPORT'}</button>
                </form>
              )}
            </div>
          </div>
          ...
          <div className="detail-row">
            <div className="detail-item">
              <div className="detail-visual">
                <img src="https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?q=80&w=2000" alt="Market Analysis" />
              </div>
              <div className="detail-text">
                <h2 className="h2-style">Strategic Market Positioning</h2>
                <p className="p-style">In the competitive crypto landscape, being unique is key. We analyze your project's technical USP (Unique Selling Proposition) and ensure it aligns with current investor trends and market liquidity.</p>
                <ul style={{listStyle: 'none', padding: 0, marginTop: '24px'}}>
                  {["Competitor Benchmarking", "Target Audience Profiling", "Liquidity Access Planning"].map((li, i) => (
            ...
            <div className="detail-item reverse">
              ...
              <div className="detail-text">
                <h2 className="h2-style">Ecosystem Growth Architecture</h2>
                <p className="p-style">We don't just launch projects; we build ecosystems. Our consultancy ensures that your token has real-world utility and a roadmap that attracts long-term holders instead of short-term speculators.</p>
                <ul style={{listStyle: 'none', padding: 0, marginTop: '24px'}}>
                  {["Utility Framework Design", "Partnership Networking", "Sustainable Scale Strategy"].map((li, i) => (
                    <li key={i} className="p-style" style={{marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '10px'}}>
                      <span style={{color: 'var(--cray-gold)', fontWeight: 800}}>✓</span> {li}
                    </li>
                  ))}
      ...
      <section className="cta-box-section">
        <div className="container-xl">
          <h2 className="h2-style" style={{color: '#000'}}>Build Your Foundation with Global Experts</h2>
          <p className="p-style" style={{color: '#555', marginTop: '15px', maxWidth: '800px', margin: '15px auto 0'}}>
            Don't risk your project's future with poor planning. Get a professional consultation from our senior strategists today.
          </p>
          <a href="#h-hero" className="form-button" style={{display: 'inline-block', width: 'auto', padding: '18px 48px', marginTop: '30px', textDecoration: 'none'}}>Request Project Audit</a>
        </div>
      </section>
      ...
      <div style={{ padding: '60px 0', textAlign: 'center', background: '#000', borderTop: '1px solid #111' }}>
        <button onClick={() => window.location.hash = '#services/token-and-blockchain-development'} className="p-style" style={{ background: 'transparent', border: '1px solid #333', color: '#888', padding: '12px 30px', borderRadius: '10px', cursor: 'pointer', textTransform: 'uppercase' }}>Back to Category</button>
      </div>
    </div>
  );
};
