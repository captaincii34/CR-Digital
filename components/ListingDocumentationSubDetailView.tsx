...
const ListingDocumentationSubDetailView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formObj = {
        "Existing Document Link": docLink || "Not provided",
        "Missing/Required Documents": missingDocs,
        "Type": "Listing Documentation Sub-Detail",
      ...
      const result = await evaluateProject('Documentation', `Link: ${docLink}. Missing: ${missingDocs}`);
      setAiResult(result);
    } catch (err: any) {
      console.error(err);
            ...
            <div className="form-card">
              ...
              {aiResult ? <div className="p-style">{aiResult.summary} <button onClick={()=>setAiResult(null)} className="form-button mt-4">Reset</button></div> : (
                <form onSubmit={handleSubmit}>
                  <p className="text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">Document Link (Optional)</p>
                  <input 
                    type="text" 
                  ...
                  />
                  <p className="text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-widest">Required Documents*</p>
                  <textarea 
                    className="w-full border p-3 rounded-lg mb-4" 
                    rows={3} 
                    placeholder="Which documents are you missing?" 
                    value={missingDocs}
                    onChange={(e) => setMissingDocs(e.target.value)}
                    required 
                  />
                  <button type="submit" disabled={loading} className="form-button">
                    {loading ? 'OPENING TELEGRAM...' : 'GET CHECKLIST'}
                  </button>
                </form>
              )}
            </div>
